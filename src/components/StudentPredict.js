...
const StudentPredict = () => {
  ...
  const GetMore = async () => {
    ...
    const res = await fetch("/api/model/image", {
      method: "POST",
      body: formData,
    }).then((res) => res.json());
  ...
  return (
    <main className="student-page">
      {isShowQuestion && (
        <Question
          image={selectedImage}
          onClose={(e) => setIsShowQuestion(false)}
        />
      )}
      <div className="left-panel">
        <div style={{ display: "flex" }}>
          <div className="btn-upload" onClick={OpenImageClick}>
            <form>
              <span>Thêm ảnh</span>
              <input
                id="open-image"
      ...
      <div className="container">
        <div className="btn-control">
          ...
          <button onClick={onViewSimilar} disabled={selectedImage == null}>
            View similar
          </button>
        </div>
        {loading ? (
          ...
          <>
            ...
            {current && (
              <div className="response">
                ...
                <p>
                  <b>Độ chính xác: </b>
                  {parseInt(current.confidence * 100)} %
                </p>
                {detail && (
                  <div style={{ textAlign: "justify" }}>
                    ...
                    <p className="hidden-detail" onClick={onHideDetail}>
                      hidden
                    </p>
                  </div>
                )}
            ...
            {similar.length > 0 && isShowSimilar && (
              <div>
                ...
                {similar.length > 0 && (
                  <div className="more-image" onClick={GetMore}>
                    Xem thêm
                  </div>
                )}
              </div>
